...
def mask_xattn_one_text_thres(img_embs, cap_i_expand, img_mask=None, i2t=True, scan=True, threshold=0.3):

    # (B_v, L_t, L_v)
    cap2img_sim = torch.bmm(cap_i_expand, img_embs.transpose(1, 2))
    #print("debug: cap2img_sim", cap2img_sim.shape)
    if scan:
        cap2img_sim = F.leaky_relu(cap2img_sim, negative_slope=0.1)
    #t2i
    # Define a threshold or percentile to determine k dynamically

    if img_mask is None:
        
        #row_sim_mean = torch.zeros(B_v, 1).to(cap2img_sim.device)
        max_sim_scores = cap2img_sim.max(dim=2).values  # (B_v, L_t)
        threshold_sim_scores = max_sim_scores - threshold*torch.abs(max_sim_scores)  # (B_v, L_t)
        row_sim_mean = torch.sum(torch.where(cap2img_sim > threshold_sim_scores.unsqueeze(-1), cap2img_sim, 0),dim=-1)
        k_all= torch.sum(cap2img_sim > threshold_sim_scores.unsqueeze(-1),dim=-1)
        row_sim_mean /= k_all
        row_sim_mean = row_sim_mean.mean(dim=1).reshape(cap2img_sim.shape[0],1)

        '''
        for b in range(B_v):
            for l_t in range(L_t):
                # Get the similarity scores for the current text token
                sim_scores = cap2img_sim[b, l_t, :]
                num_top_k = torch.sum(sim_scores > threshold_sim_scores[b, l_t])
                num_top_k = num_top_k.item() if num_top_k > 0 else 1
                token_score=sim_scores.topk(num_top_k)[0].mean()
                row_sim_mean[b,0] += token_score/L_t
        #print("debug:",row_sim_mean)
        '''
        ...
        # Add a low value to the similarity of the masked patch location 
        # to prevent it from being selected
        cap2img_sim_masked = cap2img_sim - 1000 * (1 - img_mask).unsqueeze(1)
        
        max_sim_scores = cap2img_sim_masked.max(dim=2).values  # (B_v, L_t)
        threshold_sim_scores = max_sim_scores - threshold*torch.abs(max_sim_scores)  # (B_v, L_t)        #print("debug: row_sim_mean", row_sim_mean.shape)
        row_sim_mean = torch.sum(torch.where(cap2img_sim_masked > threshold_sim_scores.unsqueeze(-1), cap2img_sim_masked, 0),dim=-1)
        k_all= torch.sum(cap2img_sim_masked > threshold_sim_scores.unsqueeze(-1),dim=-1)
        row_sim_mean /= k_all
        row_sim_mean = row_sim_mean.mean(dim=1).reshape(cap2img_sim.shape[0],1)        
        '''
        for b in range(B_v):
            for l_t in range(L_t):
                # Get the similarity scores for the current text token
                sim_scores = cap2img_sim_masked[b, l_t, :]
                
                # Determine k dynamically based on a threshold
                num_top_k = torch.sum(sim_scores > threshold_sim_scores[b, l_t])
                num_top_k = num_top_k.item() if num_top_k > 0 else 1
                #print("debug: num_top_k", sim_scores > threshold * sim_scores.max())
                token_score=sim_scores.topk(num_top_k)[0].mean()
                row_sim_mean[b,0]+= token_score/L_t
        '''
    # (B_v, 1)
    ...
    if i2t:
        ...
        if img_mask is None:
            ...
            # (B_v, 1)
            column_sim_mean = (column_sim * img_mask).sum(dim=-1, keepdim=True) / (img_mask.sum(dim=-1, keepdim=True) + 1e-8)
        #print("debug: column_sim_mean", column_sim_mean)
        sim_one_text = row_sim_mean + column_sim_mean
    else:
        sim_one_text = row_sim_mean
